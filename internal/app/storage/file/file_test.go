...
func TestFileStorage_saveToFile(t *testing.T) {
	...
	// Тестируем ошибку при создании файла - используем невалидный путь с недопустимыми символами
	originalPath := storage.filePath
	storage.filePath = filepath.Join(tmpDir, "invalid\x00path", "urls.json")
	err = storage.saveToFile()
	assert.Error(t, err)

	// Восстанавливаем оригинальный путь
	storage.filePath = originalPath
}
